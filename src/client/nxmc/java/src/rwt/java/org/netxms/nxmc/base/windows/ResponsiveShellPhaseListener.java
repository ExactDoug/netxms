...
/**
 * Installs a responsive presentation layer on the RWT management console shell.
 *
 * The controller intentionally works with the existing widget tree instead of
 * recreating views. This preserves perspective state, open tabs, selections,
 * and pinned views while changing their presentation according to the available
 * browser width.
 */
public final class ResponsiveShellPhaseListener implements PhaseListener
{
   private static final long serialVersionUID = 1L;

   private static final String CONTROLLER_ATTRIBUTE = ResponsiveShellPhaseListener.class.getName() + ".controller";

   /**
    * The render phase is the first reliable lifecycle point at which the complete
    * main window hierarchy exists, and still precedes synchronization to the client.
    */
   @Override
   ...
   public void beforePhase(PhaseEvent event)
   {
      ...
      ResponsiveShellController controller = (ResponsiveShellController)session.getAttribute(CONTROLLER_ATTRIBUTE);
      if ((controller == null) || controller.isDisposed())
      {
         controller = ResponsiveShellController.attach(display);
         if (controller != null)
            session.setAttribute(CONTROLLER_ATTRIBUTE, controller);
      }
      else
      {
         // Re-apply the current mode because pin areas and header widgets can be
         // added or removed without causing a browser resize.
         controller.refresh();
      }
   }
    ...
    */
   private static final class ResponsiveShellController
   {
      private static final Logger logger = LoggerFactory.getLogger(ResponsiveShellController.class);
      ...
      private ResponsiveShellController(Display display, Shell shell, PerspectiveSwitcher perspectiveSwitcher)
      {
       ...
       * PerspectiveSwitcher.
       */
      static ResponsiveShellController attach(Display display)
      {
         for(Shell shell : display.getShells())
         {
            ...
            if (switcher != null)
            {
               ResponsiveShellController controller = new ResponsiveShellController(display, shell, switcher);
               controller.refresh();
               return controller;
            }
         }
      ...
      void refresh()
      {
         ...
         if ((mode == ShellMode.WIDE) && (newMode != ShellMode.WIDE))
            wideSwitcherExpanded = isPerspectiveSwitcherExpanded();
         else if ((mode == ShellMode.WIDE) && (newMode == ShellMode.WIDE))
            wideSwitcherExpanded = isPerspectiveSwitcherExpanded();
      ...
      private void applyMode(boolean modeChanged)
      {
         ...
         boolean compactHeader = mode != ShellMode.WIDE;
         updateHeader(compactHeader);
